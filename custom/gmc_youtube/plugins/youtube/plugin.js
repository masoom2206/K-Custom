  ...
  var ytDialogDefinition = function (editor) {
    ...
    var dialogDefinition =
    {
              ...
              children: [
                {
				  ...
				  html : 'Example : <b>http://www.youtube.com/watch?v=iaAkWy55V3A</b>.'
                },
                {
                  // http://docs.cksource.com/ckeditor_api/symbols/CKEDITOR.dialog.definition.textInput.html
                  type : 'text',
                  id : 'txtVideoId',
                  label: 'YouTube Video Link',//'YouTube Video ID',
      ...
      onOk : function() {
		...
		var value = info.txtVideoId;
		var tag = value.replace(/:.*youtube.*/, '');

		var regexp = /^https?:\/\/www\.youtube.*?v=(.*)$/;
		var valid =  value.match(regexp) && value.match(regexp)[1];
		if(valid == null) {
			var regexp2 = /^https?:\/\/m\.youtube.*?v=(.*)$/;
			var valid2 =  value.match(regexp2) && value.match(regexp2)[1];
			if(valid2 == null) {
				alert("Invalid URL");
				return false;
			}
		  ...
		  value = value.replace(/https:.*youtube.*?v=/, '').replace(/http:.*youtube.*?v=/, '');
		  if(tag == 'https') {
			var src = 'https://youtube.com/embed/' + value + '?' + autoplay;
		  }
		  else {
			var src = 'http://youtube.com/embed/' + value + '?' + autoplay;
		  }
          // Create the iframe element
          var iframe = new CKEDITOR.dom.element( 'iframe' );
          // Add the attributes to the iframe.
          iframe.setAttributes({
            'width': 427,//info.txtWidth,
            'height': 268,//info.txtHeight,
            'type': 'text/html',
