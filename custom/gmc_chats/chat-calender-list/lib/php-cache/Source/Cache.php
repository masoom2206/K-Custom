...
require_once 'ICacheBackend.php';
...
function write($file, $content = '', $flags = 0){
	if (!file_exists($file)){
		...
		if ( !file_exists($directory) ) {
			// $mask = umask();
			$_cmd    = 'mkdir -p "' . escapeshellcmd($directory) . '" > /tmp/php_cache.log 2>&1' ;
			$ret    = null ;
			$out    = system($_cmd, $ret);
			// umask($mask);
		} else if( !is_dir($directory) ) {
		    throw new \Exception("$directory should be folder");
		}
...
class Cache {
	...
	public function __construct($root, $options = array()){
		...
		if( empty($this->options['root'] ) || $this->options['root']  == '/' ) {
		    throw new \Exception('invalid php-cache root path `' . $this->options['root'] . '` !');
		}
		$this->time = time();
